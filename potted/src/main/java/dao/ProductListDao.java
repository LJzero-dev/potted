...
public class ProductListDao {
	...
	public List<ProductInfo> getProductList(int cpage, int psize, String where, String orderBy) {	// 검색조건 들어갈 곳
		...
		String sql = "select * from t_product_info a, t_product_ctgr_big b, t_product_ctgr_small c, t_product_stock d where a.pcs_id = c.pcs_id and b.pcb_id = c.pcb_id and a.pi_id = d.pi_id and a.pi_isview = 'y' and a.pi_status = 'a'" + where + " group by a.pi_id " + orderBy + " limit " + ((cpage - 1) * psize) + ", " + psize;
		List<ProductInfo> productList = jdbc.query(sql, 
			...
			rs.getDouble("pi_dc"), rs.getString("pcb_name"), rs.getString("pcs_name"), rs.getInt("ps_stock"));
			return pi;
		});
